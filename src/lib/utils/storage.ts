...
export const getTemplatesStorage = async (): Promise<StoredTemplatesData | undefined> => {
  try {
    const data = await browser.storage.sync.get(STORAGE_KEY);
    return JSON.parse(data?.[STORAGE_KEY]);
  } catch (error) {
    console.error('An error occured while getting templates using the storage API');
    console.error(error);
  }
}
...
export const setTemplatesStorage = async (data: StoredTemplatesData) => {
  try {
    ...
    console.error('An error occured while setting templates using the storage API');
    console.error(error);
  }
}